...
function ULib.kick( ply, reason, calling_ply )
    ...
    hook.Call( ULib.HOOK_USER_KICKED, nil, steamid, reason or "[ULX] Kicked from server", calling_ply )
end
...
local function doInvis()
    ...
    for _, player in player.Iterator() do
        ...
        if t.invis then
            ...
            if player:Alive() and wep:IsValid() then
                local invis_wep = invis.wep
                if wep ~= invis_wep then

                    if invis_wep and invis_wep:IsValid() then -- If changed weapon, set the old weapon to be visible.
                        invis_wep:SetRenderMode( RENDERMODE_NORMAL )
                        invis_wep:Fire( "alpha", 255, 0 )
                        invis_wep:SetMaterial( "" )
                    end

                    invis.wep = wep
                    ULib.invisible( player, true, invis.vis )
                end
            end
        end
    end
