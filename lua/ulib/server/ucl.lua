...
function ucl.addGroup( name, allows, inherit_from, from_CAMI )
    ...
    hook.Call( ULib.HOOK_GROUP_CREATED, nil, name, ucl.groups[ name ] )
    ...
    if changed then
        for id, userInfo in pairs( ucl.authed ) do
            ...
            if ply and ply:CheckGroup( name ) then
                ULib.queueFunctionCall( hook.Call, ULib.HOOK_UCLAUTH, nil, ply ) -- Inform the masses
            end
        end

        ucl.saveGroups()

        hook.Call( ULib.HOOK_GROUP_ACCESS_CHANGE, nil, name, access, revoke )
        hook.Call( ULib.HOOK_UCLCHANGED )
    end
...
function ucl.renameGroup( orig, new )
    ...
    hook.Call( ULib.HOOK_GROUP_RENAMED, nil, orig, new )
    hook.Call( ULib.HOOK_UCLCHANGED )
...
function ucl.setGroupInheritance( group, inherit_from, from_CAMI )
    ...
    for id, userInfo in pairs( ucl.authed ) do
        ...
        if ply and ply:CheckGroup( group ) then
            ULib.queueFunctionCall( hook.Call, ULib.HOOK_UCLAUTH, nil, ply ) -- Queued so group will be changed
        end
    end
    ...
    hook.Call( ULib.HOOK_GROUP_INHERIT_CHANGE, nil, group, inherit_from, old_inherit )
    hook.Call( ULib.HOOK_UCLCHANGED )
...
function ucl.setGroupCanTarget( group, can_target )
    ...
    hook.Call( ULib.HOOK_GROUP_CANTARGET_CHANGE, nil, group, can_target, old )
...
function ucl.removeGroup( name, from_CAMI )
    ...
    hook.Call( ULib.HOOK_GROUP_REMOVED, nil, name, oldgroup )
    hook.Call( ULib.HOOK_UCLCHANGED )
...
function ucl.addUser( id, allows, denies, group, from_CAMI )
    ...
    if ply then
        ...
        hook.Call( ULib.HOOK_USER_GROUP_CHANGE, nil, id, allows, denies, group, oldgroup )
        ucl.probe( ply )
...
function ucl.userAllow( id, access, revoke, deny )
    ...
    if changed then
        ...
        hook.Call( ULib.HOOK_USER_ACCESS_CHANGE, nil, id, access, revoke, deny )
        hook.Call( ULib.HOOK_UCLCHANGED )
    end
...
function ucl.removeUser( id, from_CAMI )
    ...
    if changed then -- If the user is only added to the default garry file, then nothing changed
        ...
        hook.Call( ULib.HOOK_USER_REMOVED, nil, id, userInfo )
    end
...
function ucl.probe( ply )
    ...
    hook.Call( ULib.HOOK_UCLCHANGED )
    hook.Call( ULib.HOOK_UCLAUTH, nil, ply )
end
...
do
    ...
    hook.Add( "PlayerInitialSpawn", "ULibSendAuthToClients", function( ply )
        if ply:IsBot() then
...
hook.Add( ULib.HOOK_UCLAUTH, "ULibSendAuthToClients", function( _, ply )
    ULib.clientRPC( nil, "authPlayerIfReady", ply, ply:UserID() ) -- Call on client
end, POST_HOOK )

hook.Add( ULib.HOOK_LOCALPLAYERREADY, "ULibSendUCLDataToClient", function( ply )
...
hook.Add( "PlayerDisconnected", "ULibUCLDisconnect", function( ply )
...
hook.Add( ULib.HOOK_UCLCHANGED, "ULibSendUCLToClients", function()
    ULib.clientRPC( nil, "ULib.ucl.initClientUCL", ucl.authed, ucl.groups ) -- Send all UCL data (minus offline users) to all loaded users
    ULib.clientRPC( nil, "hook.Call", ULib.HOOK_UCLCHANGED )                -- Call hook on client
end )
...
hook.Add( "PlayerAuthed", "ULibAuth", function( ply, ... )
...
function meta:SetUserGroup( group, dontCall )
    ...
    if not dontCall and self:GetUserGroup() ~= oldGroup then -- Changed! Inform the masses of the change
        hook.Call( ULib.HOOK_UCLCHANGED )
        hook.Call( ULib.HOOK_UCLAUTH, nil, self )
    end
end
