...
hook.Add( "PlayerSay", "ULib_saycmd", function( ply, strText, bTeam )
    local match
    ...
    if match then -- We've got a winner!
        ...
        if data.__cmd then
            local return_value = hook.Call( ULib.HOOK_COMMAND_CALLED, nil, ply, data.__cmd, argv )
    ...
    return nil
end, PRE_HOOK_RETURN ) -- High-priority
